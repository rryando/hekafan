  ...
  row(type='flex', justify='center')
    Col(span="12")
      card
            ...
            Col(span=1)
              Poptip(placement="bottom")
                ...
                .api(slot='content')
                  Menu
                    MenuItem(name='2-1') 
                      a(:href="instagramUrl" target="_blank") Go to Post Link
        ...
        row
          Col(span=3)
            Avatar(shape="square" icon="ios-person" size="large" :src="'http://avatars.io/instagram/'+username")
          Col(span=18 align="left")
            row
              a(style="font-weight: 800;font-size: 18px;" :href="'https://instagram.com/'+username" target="_blank")
               | @{{username}}
        ...
        row
          iframe(:src='instagramUrl+"/embed"', width='500', height='581', style='border:none;overflow:hidden', scrolling='no', frameborder='0', allowtransparency='true', allow='encrypted-media')
          //- div(style="padding: 10px; text-align:left" v-html="getEmbedPost.html")
...
export default {
  ...
  computed: {
    ...
    instagramUrl() {
      return 'https://instagram.com/p/'+this.getShortcodeFromTag(this.post_id)
    },
    instagramEmbedUrl() {
      return `http://api.instagram.com/oembed?url=${this.instagramUrl}`
    }
  },
  mounted() {
    this.getEmbedPost();
